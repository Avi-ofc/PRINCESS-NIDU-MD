...
const premium = JSON.parse(fs.readFileSync('./AMedia/trash/avi.json'))
/**
 * Add premium user.
 * @param {String} userId
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./AMedia/trash/avi.json", JSON.stringify(_dir));
};
 ...
 * @param {Object} _dir
 */
const expiredCheck = (Avishka, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./AMedia/trash/avi.json", JSON.stringify(_dir));
			idny ? Avishka.sendMessage(idny, { text: "Your premium has run out, please buy again." }) : "";
			idny = false;
		}
	}, 1000);
};
